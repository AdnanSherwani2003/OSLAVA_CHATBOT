...
export class TraceRepository implements ITraceRepository {
  private inMemoryToolExecutions: ToolExecutionRecord[] = [];
  private inMemoryTraces: ChatTraceRecord[] = [];
  ...
  async recordToolExecution(exec: ToolExecutionRecord): Promise<void> {
    ...
    this.inMemoryToolExecutions.push({ ...exec, id });
  }
  ...
  async recordChatTrace(trace: ChatTraceRecord): Promise<void> {
    ...
    this.inMemoryTraces.push({ ...trace, id, createdAt: now });
  }
  ...
  async getToolExecutionsBySession(
    ...
    return this.inMemoryToolExecutions.filter((e) => e.sessionId === sessionId);
  }
  ...
  async getChatTracesBySession(sessionId: string): Promise<ChatTraceRecord[]> {
    ...
    return this.inMemoryTraces.filter((t) => t.sessionId === sessionId);
  }
